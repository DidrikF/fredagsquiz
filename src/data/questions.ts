 ...
 *
 * Grouped by kind — all the year rounds, then price, then sound — so the room
 * only has to learn one way of answering at a time. Keep new questions inside
 * their block; nothing enforces it.
 *
...
export const QUESTIONS: readonly Question[] = [
  ...
  },
  {
    ...
    answer: 600000,
    note: '600 000 kr for 18 kvadratmeter på hjul. Samme Leksvik-hytte som i spikerteltspørsmålet: seks ganger arealet, en sjettedel av prisen.',
  ...
  },
  {
    ...
    // The clip measures ~2 pulses a second with a spectral centroid near
    // 6 kHz — a bright rasp, not the deep metallic ping you picture when you
    // picture milking. All four options match that, so the round is decided by
    // ear and not by which one sounds like it belongs in a TINE quiz.
    options: [
      'Noen som pusser med sandpapir',
      'Håndmelking av ei ku',
      'Noen som sager med en håndsag',
      'Noen som river ost på et rivjern',
    ],
    answer: 1,
